...
def IDAstar(initial_state):
    # 初始化搜索状态
    limit = wrongmatch(initial_state)
    path = []
    closed = set()
...
def dfs(state, distance, limit, path, closed):
    # 判断是否达到距离上限
    if distance + wrongmatch(state) > limit:
        return distance + wrongmatch(state), False, []
...
print("LowerBound " + str(wrongmatch(initial_state)) + " moves")
if shortest_path == -1:
